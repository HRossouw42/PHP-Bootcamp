...
{
	if (confirm('Throw your TODO into the void?'))
	{
		console.log(str);
		$("#"+str).remove();
		deleteCookie(str);
...
{
	i++;
	$("#ft_list").append("<div "+"id="+"todo"+padZero(i)+" onclick=deleteTODO(\""+"todo"+padZero(i)+"\") "+"name="+"todo"+padZero(i)+">"+str+"</div>");
	setCookie("todo"+padZero(i), str.replace('=', '%3D').replace(';', '%3B'), 7);
}

function newTodo()
{
	var str = prompt("What do you need to get done?");
	if (str.length > 0)
		addDiv(str);
}
